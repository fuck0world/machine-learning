...
X_train, X_test, y_train, y_test = train_test_split(X_data, Y1, test_size=0.25, random_state = 33)

X_train = X_train.reshape(-1,8,8,1)
X_test = X_test.reshape(-1,8,8,1)
y_train = OneHotEncoder().fit_transform(y_train.values.reshape(-1, 1)).todense().getA()
y_test_code = OneHotEncoder().fit_transform(y_test.values.reshape(-1, 1)).todense().getA()
...
with tf.Session() as sess: 
    ...
    for epoch in range(1000): 
        ...
        if(epoch%100==0): 
            ...
            print((epoch,res))
    res_ypred = y_pred.eval(feed_dict={tf_X:X_test, tf_Y:y_test_code}).flatten() 
    # 只能预测一批样本，不能预测一个样本 
    print("测试集的预测值分别为 ", res_ypred)

from sklearn.metrics import  accuracy_score
from sklearn.metrics import mean_squared_error
 
print("预测的准确率为 ", accuracy_score(y_test, res_ypred.reshape(-1, 1)))
print("预测的均方误差为 ",  mean_squared_error(y_test, res_ypred.reshape(-1, 1)))